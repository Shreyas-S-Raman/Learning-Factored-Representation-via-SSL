...
class BlockedDoorKeyEnv(MiniGridEnv):
    """
    ## Description
    ...
    @staticmethod
    def _gen_mission():
        return "unblock the door, use the key to open the door and then get to the goal"
    ...
    def _gen_grid(self, width, height):
        ...
        goal_col = random.randint(splitIdx+1, width-2)
        goal_row = random.randint(1, height-2)
        self.put_obj(Goal(), goal_col, goal_row)

        # Place a door in the wall
        doorIdx = self._rand_int(1, height - 2)
        self.put_obj(Door("yellow", is_locked=True), splitIdx, doorIdx)
        # Place a blue obstacle in front of the door
        self.put_obj(Ball("blue"), splitIdx-1, doorIdx)
        ...
        self.mission = "unblock the door, use the key to open the door and then get to the goal"